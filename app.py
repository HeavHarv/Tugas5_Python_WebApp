...
# cara akses, misal: http://HeavHarv.pythonanywhere.com/fp_3_1_nonspark/?a=1/3
...
def fp_3_1_nonspark():
  ...
  BASE_DIR = os.path.dirname(os.path.abspath(__file__))
  url = os.path.join(BASE_DIR, "climate_data_real.csv")

  testsize = float(request.args.get('a'))
  ...
  from sklearn.linear_model import LinearRegression
  regressor = LinearRegression()
  reg_simul_nonspark = regressor.fit(X_train, y_train)

  # Simpan hasil model fit
  with open(os.path.join(BASE_DIR, "/home/HeavHarv/mysite/model/modelreg_1_nonspark.joblib.pkl"), 'wb') as f:
       joblib.dump(reg_simul_nonspark, f, compress=9)

  # Load hasil model fit
  with open(os.path.join(BASE_DIR, "/home/HeavHarv/mysite/model/modelreg_1_nonspark.joblib.pkl"), 'rb') as f:
      reg_simul_nonspark_load = joblib.load(f)

  # Predicting the Test set results
  #y_pred = regressor.predict(X_test)
  #y_pred2 = regressor.predict(X_train)

  y_pred = reg_simul_nonspark_load.predict(X_test)
  y_pred2 = reg_simul_nonspark_load.predict(X_train)
  ...
  # return render_template('MybigdataApps.html', y_aktual = list(y_train), y_prediksi = list(y_pred2), mape = mape)
  return render_template('MybigdataAppsNonPySpark.html', y_aktual = list(y_test), y_prediksi = list(y_pred), mape = mape)
...
def fp_3_2_nonspark():
  ...
  BASE_DIR = os.path.dirname(os.path.abspath(__file__))
  url = os.path.join(BASE_DIR, "house_data.csv")
  ...
  # Simpan hasil model fit
  with open(os.path.join(BASE_DIR, "/home/HeavHarv/mysite/model/modelreg_2_nonspark.joblib.pkl"), 'wb') as f:
       joblib.dump(reg_simul_nonspark, f, compress=9)

  # Load hasil model fit
  with open(os.path.join(BASE_DIR, "/home/HeavHarv/mysite/model/modelreg_2_nonspark.joblib.pkl"), 'rb') as f:
      reg_simul_nonspark_load = joblib.load(f)
...
@app.route("/fp_3_1_spark", methods=["GET", "POST"])
def fp_3_1_spark():
  # MLLIB dari Pyspark Simple Linear Regression /Klasifikasi / Clustering
  # Importing the libraries
  import numpy as np
  ...
  from pyspark.ml.feature import VectorAssembler
  # mendifinisikan Salary sebagai variabel label/predictor
  dataset = data.select(data.Population, data.CO2_ppm, data.CH4_ppb, data.N20_ppb,data.TemperatureDiff.alias('label'))
  # split data menjadi 70% training and 30% testing
  ...
  y_aktual = testSet.select("label")
  y_pred2 = testSet.select("prediction")
  # y_pred2.show()


  return render_template('MybigdataAppsPySpark.html', y_aktual = y_aktual.rdd.flatMap(lambda x: x).collect(), y_prediksi = y_pred2.rdd.flatMap(lambda x: x).collect(), mape = evaluator.evaluate(testSet, {evaluator.metricName: "r2"}))

@app.route("/fp_3_2_spark", methods=["GET", "POST"])
def fp_3_2_spark():
  # MLLIB dari Pyspark Simple Linear Regression
  # Importing the libraries
  import numpy as np
  import matplotlib.pyplot as plt
  import pandas as pd
  ...
  from pyspark.ml.feature import VectorAssembler
  # mendifinisikan Salary sebagai variabel label/predictor
  dataset = data.select(data.bedrooms,data.bathrooms,data.sqft_living,data.sqft_lot,data.floors,data.waterfront,data.view,data.condition,data.grade,data.sqft_above,data.yr_built,data.price.alias('label'))
  # split data menjadi 70% training and 30% testing
  training, test = dataset.randomSplit([0.7, 0.3], seed = 100)
  # mengubah fitur menjadi vektor
  assembler = VectorAssembler().setInputCols(['bedrooms',	'bathrooms',	'sqft_living',	'sqft_lot',	'floors',	'waterfront',	'view',	'condition','grade','sqft_above','yr_built']).setOutputCol('features')
  trainingSet = assembler.transform(training)
  # memilih kolom yang akan di vektorisasi
  trainingSet = trainingSet.select("features","label")
